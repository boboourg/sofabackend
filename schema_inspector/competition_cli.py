"""CLI for loading competition-family data into PostgreSQL."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Fetch competition-family Sofascore endpoints and persist them into PostgreSQL.",
    )
    parser.add_argument("unique_tournament_id", type=int, help="Unique tournament ID from Sofascore.")
    parser.add_argument("--season-id", type=int, default=None, help="Optional season ID for /season/{id}/info.")
...
async def _run(args: argparse.Namespace) -> int:
    ...
    print(
        ...
        f"season_id={result.season_id if result.season_id is not None else '-'} "
        f"unique_tournaments={result.written.unique_tournament_rows} "
        f"seasons={result.written.season_rows} "
