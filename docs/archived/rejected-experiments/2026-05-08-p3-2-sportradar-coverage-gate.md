# REJECTED EXPERIMENT: P3.2 — Sportradar Coverage Gate

**Status**: REJECTED 2026-05-08 (rolled back via commit 7c4ef67)
**Reason**: Historical simulation against 30 days of football api_request_log
showed an aggregate **false-positive rate of 69.23%** (target was <0.5%).

Per-endpoint FP breakdown:

  /event/{e}/incidents              99.8% FP
  /event/{e}/statistics             98.9% FP
  /event/{e}/lineups                96.0% FP
  /season/.../top-teams/overall     69.8% FP
  /season/.../top-players/overall   69.7% FP
  /season/.../top-players-per-game  60.4% FP
  /season/.../player-of-the-season   5.0% FP   ← only viable

**Fundamental finding**: the hypothesis Sofascore-data-availability ≈
Sportradar-coverage-matrix is FALSE. The SR matrix describes only what
the Sportradar API itself delivers; Sofascore augments with multiple
other sources (Opta, Stats Perform, internal scouts, manual editorial
input). matrix=0 does NOT imply Sofascore returns 404 — it returns 200
in 69% of these cases.

Top dangerous UTs (matrix=0, Sofascore=200, blind_id confidence=1.0):

  205   FNL (Russia 2nd-tier)        — 5,723 FP/30d
  1596  Copa do Nordeste (Brazil)    — 5,523 FP/30d
  1139  Première Ligue Féminine     — 3,659 FP/30d
  937   Botola Pro (Morocco)         — 3,632 FP/30d
  649   Chinese Super League         — 3,623 FP/30d
  808   Egyptian Premier League      — 2,800 FP/30d

Gate was never enabled in prod — kill-switch defaulted to false throughout
B-1, B-2, B-3 deploy. No data loss occurred.

The original design doc + audit utility are kept here as historical
record. Do NOT revive without solving the data-source gap (see
the Альтернативы section at the end of the design doc body).
