...
| 2026-05-15 02:45 | **85%** | 0/24h | Commit `8bfc340` — fix `/api/v1/unique-tournament/{utid}/seasons` shape regression for tournaments without raw snapshots. Previously generic fallback emitted `{"seasons":[{"seasonId":..., "uniqueTournamentId":...}]}` from the junction table directly; new dedicated handler JOINs to `season` and emits the canonical `{"seasons":[{"id":..., "name":..., "year":..., "editor":bool}]}`. Verified on prod: UT=7 returns 23 properly-shaped seasons; UT=8 (LaLiga, snapshot hit) still returns 57 seasons via normal waterfall (regression check). Shape % is hard to quantify but at least 3 of 12 sport-class UTs were affected before fix. |
